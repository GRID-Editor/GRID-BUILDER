...
if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
  PRODUCT_NAME="GRID - Insiders"
  PRODUCT_CODE="GRIDInsiders"
...
if [[ -z "${1}" ]]; then
	OUTPUT_BASE_FILENAME="GRID-${VSCODE_ARCH}-${RELEASE_VERSION}"
else
	OUTPUT_BASE_FILENAME="GRID-${VSCODE_ARCH}-${1}-${RELEASE_VERSION}"
fi
